...
namespace ply
{
  ...
  void leer_vertices(unsigned num_vertices, vector<float> &vertices,
                     ifstream &src, vector<double> &coordenadaTextura);
  void leer_caras(unsigned num_vertices, unsigned num_caras,
                  vector<int> &caras, ifstream &src);

  bool tiene_coordenadas_textura = false;

  //**********************************************************************
  // funcion principal de lectura

  void read(const char *nombre_archivo_pse,
            vector<float> &vertices, vector<int> &caras, vector<double> &coordenadaTextura)
  {
    // modelos ply en:
    ...
    leer_vertices(num_vertices, vertices, src, coordenadaTextura);
    leer_caras(num_vertices, num_caras, caras, src);

    cout << "archivo ply leido." << endl
         << flush;
  }

  //**********************************************************************

  void read_vertices(const char *nombre_archivo_pse, vector<float> &vertices, vector<double> &coordenadaTextura)
  {
    ...
    cout << "archivo ply leido (únicamente vértices)" << endl
         << flush;
  }
                     ...
                     ifstream &src, vector<double> &coordenadaTextura)
  {
    ...
    if (tiene_coordenadas_textura)
    {
      coordenadaTextura.resize(num_vertices * 2);
    }
    ...
    for (long long iv = 0; iv < num_vertices; iv++)
    {
      ...
      // printf("tiene_coordenadas_textura: %d\n", tiene_coordenadas_textura);
      // añadir coordenadas de textura
        if (tiene_coordenadas_textura)
        {
            // Guardar la precisión actual del flujo
            std::streamsize prev_precision = src.precision();
            // Establecer la precisión del flujo a 8 decimales
            src.precision(8);
            ...
            cout << "texture coords #" << iv << " readed: (" << s << "," << t << ")" << endl;

            coordenadaTextura[base + 0] = s; // s
            coordenadaTextura[base + 1] = t; // ts

            // Restaurar la precisión original del flujo
            src.precision(prev_precision);
        }

      src.getline(buffer, tam_buffer);
    }
    tiene_coordenadas_textura = false;
  ...
  */
                     ...
                     unsigned &num_caras, const bool lee_num_caras)
  {
    ...
    while (en_cabecera)
    {
      ...
      else if (token == "property")
      {
        ...
        if (token == "float")
        {
          ...
          if (token == "s" || token == "t")
          {
            tiene_coordenadas_textura = true;
          }
        }
      }
    ...
    cout << "  tiene coordenadas de textura: " << (tiene_coordenadas_textura ? "sí" : "no") << endl;
